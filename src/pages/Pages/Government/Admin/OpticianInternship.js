...
function OpticianInternship() {
    ...
    const handleAddComment = async () => {
        ...
        const data = { trainingId: idx, note: comment }

        try {
            setLoading(true);
            setmodal_list(false);
            const url = `trainings/approve?role=${user.type.trim()}&senderid=${user.id}`;
    ...
    const fetchTrainings = useCallback(async (page) => {
        setTab_two(' ');
        setTab_three(' ');
        setTab_one('text-success');

        const p = page || 1;
        const url = `trainings?limit=5&page=${p}`;

        try {
            const rs = await request(url, 'GET', true);
            isRenderSearch.current.style.display = 'none';
            isRenderTraining.current.style.display = '';
                    ...
                    <td>
                        <div className="hstack  flex-wrap">
                            ...
                            {/* <button type="button" className="btn btn-ghost-secondary btn-icon btn-sm fs-16" id="Tooltip2">
                                <i onClick={() => handleUpdateTraining(e.user.training, e.user.training.id)}
                                    className="ri-download-2-line fs-17 lh-1 align-middle"></i>
                            </button> */}
                            <Link to={`/admin-dashboard-it/${`training`}/${e.id}`} className="link-success btn-icon btn-sm" id="Tooltip3"><i className="ri-compass-3-line fs-16"></i></Link>
                        ...
                        <div className="hstack flex-wrap">
                            <button href="#" type="button" className="btn btn-ghost-secondary btn-icon btn-sm fs-16"
                                id="Tooltip1"><i className="ri-edit-2-line"
                                    onClick={() => tog_list(e.id)}
                                ></i></button>
                            <Link to={`/admin-dashboard-if/${`training`}/${e.id}`} className="link-success btn-icon btn-sm" id="Tooltip3"><i className="ri-compass-3-line fs-16"></i></Link>
    ...
    const handlePagination = page => {
        fetchTrainings(page.selected + 1);
                    ...
                    <div className="card">
                        <div className="card-header align-items-center d-flex">
                            ...
                            <div className="flex-shrink-0">
                                <div className="">
                                    <Link className={`mx-3 ${tab_one}`} to="#" onClick={() => fetchTrainings()}>All</Link>
...
export default OpticianInternship