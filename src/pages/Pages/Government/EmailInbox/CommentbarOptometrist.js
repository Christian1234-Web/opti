...
const CommentToolbar = ({ messages, userId }) => {
    ...
    const showMessage = async comment => {
        setMessage(comment);
        toggleRightCanvas();
    }
                ...
                <div className="p-4 pb-0">
                    ...
                    <SimpleBar className="message-list-content mx-n4 px-4 message-list-scroll">
                        <ul className="message-list">
                            {messages?.length < 1 || messages?.internship === null ? '' : <li className=''>
                                <div className="col-mail col-mail-1">
                                    ...
                                    <Link to="#" className="title text-capitalize">{new Date(messages?.internship?.createdAt).toDateString()}</Link>
                                </div>
                                <div className="col-mail col-mail-2" onClick={() => showMessage(messages?.internship)}>
                                    <Link to="#" className="subject text-capitalize"> {<span className={"badge me-2 bg-"}></span>} <span className="teaser text-capitalize">{`Internship ID:`} {messages?.internship?.id}</span>
                                    </Link>
                                    <div className="date">{new Date(messages?.internship?.createdAt).toLocaleDateString()}</div>
                                </div>
                            </li>}

                            {messages?.length < 1 || messages?.optometrist === null ? '' : <li className=''>
                                <div className="col-mail col-mail-1">
                                    {/* <div className="form-check checkbox-wrapper-mail fs-14">
                                                <input className="form-check-input" type="checkbox" value="" id={item.forId} />
                                                <label className="form-check-label" htmlFor={item.forId}></label>
                                    ...
                                    <Link to="#" className="title text-capitalize">{new Date(messages?.optometrist?.createdAt).toDateString()}</Link>
                                </div>
                                <div className="col-mail col-mail-2" onClick={() => showMessage(messages?.optometrist)}>
                                    <Link to="#" className="subject text-capitalize"> {<span className={"badge me-2 bg-"}></span>} <span className="teaser text-capitalize">{`Optician ID:`} {messages?.optometrist?.id}</span>
                                    </Link>
                                    <div className="date">{new Date(messages?.optometrist?.createdAt).toLocaleDateString()}</div>
                                </div>
                            </li>}
