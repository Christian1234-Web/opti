...
function WizardOptometrist(props) {
    const store = useContext(Store);
    // { optometrists, user, existPageOptometrist, oneOptometrist, user_date_of_orientation_optometrist, setUser_date_of_orientation_optometrist, user_school_optometrist, setUser_school_optometrist, }
    ...
    const addSupervisor = () => {
        ...
        setSupervisor(x);
        // console.log(x, data, props.supervisor_first_name);
    }
    ...
    const createInternship = async () => {
        ...
        const data = {
            userId: user.id, schoolAttended: props.user_school_optometrist, dateOfOrientation: props.user_date_of_orientation_optometrist, supervisors: supervisor, status: 'Pending',
        ...
        try {
            ...
            setError('Successful');
            setLoading(false);
            history.push(`/optometrist-dashboard/internship/${rs?.data?.id}`);
            // handleSuccess();
            // props.existPageOptometrist();
        } catch (err) {
            setLoading(false);
    ...
    const createInternshipAndExit = async () => {
        ...
        const data = {
            userId: user.id, schoolAttended: props.user_school_optometrist, dateOfOrientation: props.user_date_of_orientation_optometrist, supervisors: supervisor, status: 'Continue',
            hospitals: hospital, documents: allFiles
        }
    ...
    const updateInternship = async () => {
        ...
        try {
            setLoading(true);
            const url_sup = `trainings/supervisor/${props.oneOptometrist.supervisors[0].id}?senderid=${user.id}`;
            const url_hos = `trainings/hospital/${props.oneOptometrist.hospitals[0].id}?senderid=${user.id}`;
            const url = `internships/update/${props.oneOptometrist.id}?senderid=${user.id}`;
            // console.log(url);
            const rsh = await request(url_hos, 'PATCH', true, dataHos);
            const rss = await request(url_sup, 'PATCH', true, dataSup);
            const rs = await request(url, 'PATCH', true, data);
            const fs = await request(`documents/upload?name=internship&id=${props.oneOptometrist?.id}`, 'POST', true, docu);
            setError('Successful!');
            setFiles([]);
            setLoading(false);
            handleSuccess();
    ...
    const fetchUserDetails = useCallback(async () => {
        try {
            setLoading(true);
            const url = `internships/${props.oneOptometrist?.id}`;
    ...
    }, [props.oneOptometrist?.id, setDocuments])
                                            ...
                                            <Col lg={6}>
                                                <div className="mb-3">
                                                    ...
                                                    <Input
                                                        ...
                                                        value={props.user_school_optometrist}
                                                        onChange={(e) => props.setUser_school_optometrist(e.target.value)}
                                            ...
                                            <Col lg={6}>
                                                <div className="mb-3">
                                                    ...
                                                    <Flatpickr
                                                        ...
                                                        style={{ background: '#fff' }}
                                                        value={props.user_date_of_orientation_optometrist}
                                                        onChange={e => props.setUser_date_of_orientation_optometrist(e)}
                                                    />
